...
tvc/dbsnp/%/TSVC_variants.vcf : bam/%.bam
	...
	-s $(DBSNP_TARGETS_INTERVALS) -i $< -r $(REF_FASTA) -o $(@D) -N 4 \
...
tvc/vcf/$1/TSVC_variants_final.vcf : bam/$1.bam bam/$1.bai
	$$(call LSCRIPT_PARALLEL_MEN,4,$$(RESOURCE_REQ_MEDIUM_MEM),$$(RESOURCE_REQ_VSHORT),"$$(LOAD_JAVA8_MODULE); \
	$$(TVC) -i $$< -r $$(REF_FASTA) -o $$(@D) -N 4 \
	$$(if $(TARGETS_FILE_INTERVALS),-b $$(TARGETS_FILE_INTERVALS)) -m $$(TVC_MOTIF) \
	-t $$(TVC_ROOT_DIR) --primer-trim-bed $$(PRIMER_TRIM_BED) && \
	$$(call GATK,LeftAlignAndTrimVariants,$$(RESOURCE_REQ_MEDIUM_MEM)) -R $$(REF_FASTA) \
	--variant $$(@D)/TSVC_variants.vcf -o $$(@D)/TSVC_variants.vcf.tmp --splitMultiallelics && \
